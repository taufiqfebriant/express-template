...
import { isInvalidInput, mapRelation, formUniqueKey, kvDb2Col, setAuditData } from './t4t-utils.js';
...
const remove = async (req, res) => {
  ...
  if (table.deleteLimit > 0 && ids.length > table.deleteLimit)
    return res.status(400).json({ error: `Select up to ${tabledeleteLimit} items` });
