...
export async function readGoogle(filename) {
  try {
    const res = await fetch('/api/app-sample/tests/gcp-sign', {
      method: 'POST',
      headers,
...
export async function uploadMemory(files) {
  try {
    ...
    const res = await fetch('/api/app-sample/tests/upload-memory', { method: 'POST', body: formData });
    const rv = await res.json();
    console.log(rv);
  } catch (e) {
    console.log(e);
...
export async function uploadFiles(files) {
  try {
    ...
    const res = await fetch('/api/app-sample/tests/upload-disk', { method: 'POST', body: formData });
...
export async function jsonOnly() {
  await fetch('/api/app-sample/tests/upload-disk', {
    method: 'POST',
    headers: {
